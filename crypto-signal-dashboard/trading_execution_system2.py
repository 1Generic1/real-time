...
class MLEnhancedTradingSystem(TradingExecutionSystem):
    ...
    def generate_enhanced_signal(self, ta_signals, onchain_signals, price_data, onchain_data):
        ...
        return self.determine_trade_direction(combined_score), ml_signals, combined_score


def integrate_and_trade_with_ml(symbol='BTC/USDT', timeframe='4h', account_balance=1000, enable_ml=True):
    ...
    if enable_ml and ML_AVAILABLE and trading_system.ml_trained:
        trade_direction, ml_signals, combined_score = trading_system.generate_enhanced_signal(
            ta_signals, onchain_signals, price_data, all_metrics
        )
        print(f"\n🎯 COMBINED SCORE: {combined_score} (Traditional + ML)")
        
        # Generate the actual trade signal using the direction
        if trade_direction != "NO_TRADE":
            trade_signal = trading_system.generate_trading_signal(ta_signals, onchain_signals, price_data, symbol)
        else:
            trade_signal = None
    else:
