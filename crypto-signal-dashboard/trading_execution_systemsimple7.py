# trading_execution_systemsimple7.py - CORRECTED VERSION WITH FIXED LIQUIDATION CALCULATION
...
def get_cached_or_fetch_price_data(symbol, timeframe, limit, cache_minutes=5):
    ...
    if cache_key in price_data_cache:
        ...
        if time_diff < cache_minutes * 60:
            print(f"   📊 Using cached data from {cache_time.strftime('%H:%M:%S')} ({time_diff:.0f}s ago)")
            return cached_data.copy()
        else:
            print(f"   📊 Cache expired ({time_diff:.0f}s > {cache_minutes*60}s), fetching fresh...")
    ...
    if data is not None:
        ...
        print(f"   📊 Cached new data at {current_time.strftime('%H:%M:%S')}")
        return data.copy()
    return None
...
def get_cached_onchain_data(symbol):
    ...
    if cache_key in ONCHAIN_CACHE:
        ...
        if time_diff < ONCHAIN_CACHE_DURATION:
            print(f"   ⛓️ Using cached on-chain data ({time_diff:.0f}s ago)")
            return cached_data.copy()
    
    # If not cached or expired, return None (will be fetched fresh)
    return None
...
class TradingExecutionSystem:
    ...
    def calculate_position_size(self, entry_price, stop_loss_price, symbol, leverage=None, price_data=None):
        ...
        print(f"🔍 DIRECTION DEBUG: Entry=${entry_price:,.2f}, Stop=${stop_loss_price:,.2f}")
        print(f"   Entry > Stop? {entry_price > stop_loss_price} ({entry_price:,.2f} > {stop_loss_price:,.2f})")
        print(f"   Entry < Stop? {entry_price < stop_loss_price} ({entry_price:,.2f} < {stop_loss_price:,.2f})")
        print(f"   Direction determined: {'LONG' if entry_price > stop_loss_price else 'SHORT'}")
        ...
        if direction == "LONG":
            ...
            # Liq = Entry × (1 - (Initial_Margin - Maintenance_Margin))
        ...
        if direction == "LONG" and liquidation_price > entry_price:
            print(f"⚠️  WARNING: Calculated liquidation price (${liquidation_price:,.2f}) is ABOVE entry (${entry_price:,.2f}) for LONG position")
            print(f"   Using fallback calculation: 15% below entry")
            liquidation_price = entry_price * 0.85  # Fallback: 15% below entry
            
        elif direction == "SHORT" and liquidation_price < entry_price:
            print(f"⚠️  WARNING: Calculated liquidation price (${liquidation_price:,.2f}) is BELOW entry (${entry_price:,.2f}) for SHORT position")
            print(f"   Using fallback calculation: 15% above entry")
    ...
    def score_signals(self, ta_signals, onchain_signals):
        ...
        # Technical Analysis Scoring - IMPROVED MATCHING LOGIC
        for signal in ta_signals:
            ...
            if signal_score == 0:
                ...
                if any(keyword in signal_upper for keyword in bullish_keywords):
                    # Give stronger weight for specific patterns
            ...
            # Debug output
            emoji = "📗" if signal_score > 0 else "📕" if signal_score < 0 else "📊"
            print(f"  {emoji} {signal[:45]:45} = {signal_score:+2} ({matched_type})")
        
        print(f"\n  Technical Subtotal: {score}")
        
        # On-Chain Scoring - CONSISTENT LOGIC
        print("\n  On-Chain Signals:")
        for signal in onchain_signals:
            signal_upper = signal.upper()
            
            # CONSISTENT on-chain scoring
            if 'BEARISH' in signal_upper and ('🔴' in signal or 'HIGH' in signal_upper or 'CAUTION' in signal_upper):
                score -= 2
                print(f"  🔴 {signal[:45]:45} = -2")
            elif 'BULLISH' in signal_upper and ('🟢' in signal or 'LOW' in signal_upper):
                score += 2
                print(f"  🟢 {signal[:45]:45} = +2")
            elif 'NEUTRAL' in signal_upper or 'NORMAL' in signal_upper or '🟡' in signal or 'BALANCED' in signal_upper:
                print(f"  🟡 {signal[:45]:45} = +0")
            else:
                # Default: ignore or give minimal weight
                print(f"  ⚪ {signal[:45]:45} = +0")
        
        print("-" * 60)
        print(f"🎯 FINAL TRADITIONAL SCORE: {score}")
        print("-" * 60)
    ...
    def calculate_levels(self, direction, price_data, current_price, symbol):
        """Calculate entry, stop loss, and take profit for BOTH LONG and SHORT with MINIMUM STOPS"""
    ...
    def generate_trading_signal(self, ta_signals, onchain_signals, price_data, symbol, custom_leverage=None):
        ...
        # REJECT trades with stops that are too small (leads to oversized positions)
        MIN_STOP_PCT = 1.5  # Minimum 1.5% stop
        if stop_distance_pct < MIN_STOP_PCT:
            print(f"⚠️  Stop distance too small: {stop_distance_pct:.1f}% < {MIN_STOP_PCT}%")
            print(f"   Trade rejected - stop too tight for proper position sizing")
            return None
        ...
        print(f"   Stop Distance: {stop_distance_pct:.1f}%")
        print(f"   Risk Amount Target: ${self.account_balance * self.risk_per_trade:.2f}")
        ...
        # Verify the risk is correct
        if risk_amount < (self.account_balance * self.risk_per_trade * 0.8):  # Less than 80% of target
            print(f"⚠️  Risk amount too small: ${risk_amount:.2f} (should be ~${self.account_balance * self.risk_per_trade:.2f})")
            print(f"   Stop distance: {stop_distance_pct:.1f}% may be too large")
        ...
        trade_signal = {
            ...
            'stop_distance_pct': stop_distance_pct  # Add stop distance for debugging
        }
...
class MLEnhancedTradingSystem(TradingExecutionSystem):
    ...
    def train_ml_models(self, price_data, symbol=None):
        """Train ML models with consistency"""
        if not ML_AVAILABLE:
        ...
        if (self.ml_trained and self.training_symbol == symbol and 
            ...
            if time_since_training < 300:  # 5 minutes
                print(f"✅ ML MODELS ALREADY TRAINED (trained {time_since_training:.0f}s ago)")
                return True
...
def integrate_and_trade_with_ml(symbol='BTC/USDT', timeframe='4h', account_balance=1000, enable_ml=True, custom_leverage=None):
    """Main function with ML enhancement - WITH TIMESTAMPS AND CONSISTENCY"""
    ...
    if enable_ml and ML_AVAILABLE and not trading_system.ml_trained:
        print(f"\n🤖 TRAINING SIMPLE ML MODELS ON {symbol} DATA...")
    ...
    for i, crypto in enumerate(cryptos, 1):
        ...
        try:
            ...
            if enable_ml and ML_AVAILABLE:
                print(f"   🤖 Creating ML system for {crypto}...")
            ...
            if enable_ml and ML_AVAILABLE and isinstance(trading_system, MLEnhancedTradingSystem):
                print(f"   🤖 Training ML specifically for {crypto}...")
            ...
            if trade_signal:
                ...
                print(f"   {direction_emoji} TRADE FOUND: {trade_signal['direction']} - Score: {score_to_show}")
            ...
            try:
                ...
                if current_data is not None:
                    ...
                    if abs(price_change) > 1.0:  # If price moved >1%
                        print(f"   ⚠️  SIGNAL AGED: Price moved {price_change:+.2f}%")
...
def test_scoring_consistency():
    ...
    if len(unique_scores) == 1:
        print(f"✅ PERFECT CONSISTENCY! All scores are identical: {scores[0]}")
        return True
    else:
...
def test_final_consistency():
    ...
    print("\n" + "="*70)
    print("✅ FINAL CONSISTENCY VERIFICATION TEST")
    print("="*70)
    ...
    for signal, expected in test_cases:
        ...
        print(f"  {signal[:40]:40} → Score: {signal_score:+2} ({matched_type})")
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 INITIATING CRYPTO ANALYSIS ENGINE (SEED: {SEED})...")
    time.sleep(1)
    ...
    print("\n" + "="*70)
    print("🔍 PRICE CONSISTENCY DIAGNOSTIC TEST")
    print("="*70)
    
    # Test 1: Fetch ETH once
    print("Test 1: Fetching ETH data (100 candles)...")
    eth1 = get_cached_or_fetch_price_data('BTC/USDT', '4h', ML_TRAINING_DATA_LENGTH)
    if eth1 is not None:
        print(f"   Price 1: ${eth1['close'].iloc[-1]:,.2f}")
        
        # Test 2: Fetch ETH immediately again (should use cache)
        print(f"\nTest 2: Fetching ${Symbol} data again (should use cache)...")
        eth2 = get_cached_or_fetch_price_data('BTC/USDT', '4h', ML_TRAINING_DATA_LENGTH)
        if eth2 is not None:
            print(f"   Price 2: ${eth2['close'].iloc[-1]:,.2f}")
            
            # Check if they're the same
            price_diff = abs(eth1['close'].iloc[-1] - eth2['close'].iloc[-1])
            if price_diff < 0.01:  # Less than 1 cent difference
                print(f"✅ CACHE WORKING: Prices are identical (difference: ${price_diff:.4f})")
            else:
                print(f"⚠️  CACHE ISSUE: Prices differ by ${price_diff:.2f}")
    
    print("\n" + "="*70)
    print("🧪 TEST 1: SINGLE SYMBOL ANALYSIS WITH ML")
    print("="*70)
    integrate_and_trade_with_ml('BTC/USDT', '4h', account_balance=1000, enable_ml=True)
    
    time.sleep(2)
    
    print("\n" + "="*70)
    print("🧪 TEST 2: BATCH ANALYSIS WITH ML (NOW SHOULD BE CONSISTENT)")
    print("="*70)
    batch_analyze_cryptos(['ETH/USDT', 'SOL/USDT'], enable_ml=True)
    
    time.sleep(2)
    
    print("\n" + "="*70)
    print("🧪 TEST 3: BATCH ANALYSIS WITHOUT ML FOR COMPARISON")
    print("="*70)
    batch_analyze_cryptos(['ETH/USDT', 'SOL/USDT'], enable_ml=False)
