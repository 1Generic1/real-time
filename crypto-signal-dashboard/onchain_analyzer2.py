...
def display_key_levels(current):
    ...
    if current is not None:
        ...
        # Moving Averages
        if 'ema_20' in current and not pd.isna(current['ema_20']):
            ema20_pct = ((current['close'] - current['ema_20']) / current['close'] * 100)
            print(f"  • EMA 20: ${current['ema_20']:,.2f} ({ema20_pct:+.1f}%)")
        
        if 'ema_50' in current and not pd.isna(current['ema_50']):
            ema50_pct = ((current['close'] - current['ema_50']) / current['close'] * 100)
            print(f"  • EMA 50: ${current['ema_50']:,.2f} ({ema50_pct:+.1f}%)")
