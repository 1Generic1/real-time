# trading_execution_systemsimple5_fixed.py
import pandas as pd
import numpy as np
...
def batch_analyze_cryptos(cryptos=None, account_balance=1000, enable_ml=False):
    """Analyze multiple cryptocurrencies with TIMESTAMPS"""
    if cryptos is None:
    ...
    # Create trading system WITHOUT ML for consistency
    trading_system = TradingExecutionSystem(account_balance=account_balance)
    print("📊 TRADITIONAL SCANNING ONLY (for consistency)")
    
    best_opportunities = []
    analysis_times = []  # Track timing for each crypto
    ...
    for i, crypto in enumerate(cryptos, 1):
        ...
        try:
            ...
            # Generate trade signal (NO ML for consistency)
...
if __name__ == "__main__":
    ...
    # Single symbol analysis with ML enhancement
    integrate_and_trade_with_ml('ETH/USDT', '4h', account_balance=1000, enable_ml=True)
    
    # Multi-crypto scanning WITHOUT ML for consistency
    time.sleep(2)
    print("\n" + "="*70)
    print("⚠️  RUNNING MULTI-SCAN WITHOUT ML FOR CONSISTENT COMPARISON")
    print("="*70)
